import os
import pickle
from typing import List, Union, Optional, Dict, Any #for type hinting
import time
import numpy as np
import pandas as pd 
...
from deepface.commons import logger as log #for logging messages
...
def find(
    ...
    # creating the db represntaion in a unique way ( i modify it to make it simple we can)
    file_parts = [
        "DS",
        " Using",
        model_name,
    ...
    # this part to create the pkl file
    ...
    df = pd.DataFrame(representations) #Convert representations to a DataFrame.
...
# a helper function to find embeddings for multiple images.
def __find_bulk_embeddings(
    employees: List[str], # list of images
    ...
    for employee in tqdm(
        ...
        file_hash = image_utils.find_image_hash(employee) # Compute a hash for each image
